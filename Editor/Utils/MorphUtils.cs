...
namespace AnimConverter.Editor.Utils
{
    public static class MorphUtils
    {
        ...
        /// <param name="vrmBlendShapeMapping">VRM标准映射表</param>
        public static void InitializeMorphMapping(
            List<VMDMorphFrame> vmdMorphFrames,
            List<string> availableMorphs,
            Dictionary<string, string> morphMapping,
            Dictionary<string, string> vrmBlendShapeMapping,
            Dictionary<string, bool> selectedMorphs
            )
        {
            // logging
            ...
            {
                // 优先使用VRM标准映射
                if (vrmBlendShapeMapping != null &&
                    vrmBlendShapeMapping.TryGetValue(vmdMorph, out var vrmName) &&
                    availableMorphs.Contains(vrmName))
                {
                    morphMapping[vmdMorph] = vrmName;
